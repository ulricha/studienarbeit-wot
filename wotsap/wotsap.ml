...
module Wotsap_read = struct
  ...
  let names = DynArray.make 41000;;
  let keyids = DynArray.make 41000;;
  let signatures = DynArray.make 41000;;
  read_wotsap_file inc names keyids signatures;;
  close_in inc;;

  let keys = DynArray.make 41000;;
  for i = 0 to (DynArray.length names)-1 do
    DynArray.add keys ((DynArray.get keyids i), (DynArray.get names i))
  done;
end;;
