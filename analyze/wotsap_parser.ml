...
let read_wotsap_file fname =
  let names = DynArray.make 42000 in
  let keyids = DynArray.make 42000 in
  let signatures = DynArray.make 42000 in
  ...
  let input = IO.input_channel inc in
	...
	assert ((DynArray.length names) = (DynArray.length keyids));
