...
module Make(G : Sig.I) = struct
  ...
  let largest_component_as_graph scc_list original_graph =
    let sorted_list = list_list_sort_reverse scc_list in
    let largest = List.hd sorted_list in
      graph_from_node_list largest original_graph

  let scc_list_to_graph_list scc_list original_graph =
    let convert scc = graph_from_node_list scc original_graph in
      List.map convert scc_list
